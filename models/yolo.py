...
class MultiScaleConv(nn.Module):
    ...
    def forward(self, x):
        ...
        out5 = self.conv5(x)
        out7 = self.conv7(x)
        out = out3 + out5 + out7  # 或者 torch.cat([out3, out5, out7], dim=1) 看需求
        return self.act(out)
...
class DualDDetectMultiScale(DualDDetect):
    # Dual DFL detection head with multi-scale convolution blocks.
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c4, c5 = make_divisible(max((ch[self.nl] // 4, self.reg_max * 4, 16)), 4), max((ch[self.nl], min((self.nc * 2, 128))))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(MultiScaleConv(x, c2), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch[:self.nl])
        self.cv3 = nn.ModuleList(
            nn.Sequential(MultiScaleConv(x, c3), nn.Conv2d(c3, self.nc, 1)) for x in ch[:self.nl])
        self.cv4 = nn.ModuleList(
            nn.Sequential(MultiScaleConv(x, c4), nn.Conv2d(c4, 4 * self.reg_max, 1)) for x in ch[self.nl:])
        self.cv5 = nn.ModuleList(
            nn.Sequential(MultiScaleConv(x, c5), nn.Conv2d(c5, self.nc, 1)) for x in ch[self.nl:])
        self.dfl = DFL(self.reg_max)
        self.dfl2 = DFL(self.reg_max)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, DualDDetectMultiScale, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
